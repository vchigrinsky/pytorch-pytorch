...
class PersistentCache:
    def __init__(self):
        ...
        self.system["hash"] = hashlib.sha256(json.dumps(self.system, sort_keys=True).encode("utf-8")).hexdigest()

        self.local_cache_path = os.path.join(cache_dir(), "cache")
        self.global_cache_path = os.path.join(os.path.dirname(config.global_cache_dir), self.system["hash"]) if config.global_cache_dir is not None else None
    ...
    def update_local_cache(self, local_cache):
        write_atomic(self.local_cache_path, json.dumps({"system": self.system, "cache": local_cache}, indent=4))

...
class DynamoConfig:
    ...
    constant_functions: Dict[ModuleType, bool] = dataclasses.field(default_factory=lambda: {
    ...
    skipfiles_inline_module_allowlist: Set[ModuleType] = dataclasses.field(default_factory=lambda: {
        torch.nn,
        torch.distributions,
        torch.testing,
        torch.ao.nn,
    ...
    # FX IR.
    allowed_functions_module_string_ignorelist: Set[str] = dataclasses.field(default_factory=lambda: {
        "torch.distributions",
        "torch.testing",
        "torch._refs",
        "torch._prims",
    ...
    def setup_debug_dir(self):
        if self.DEBUG_DIR_VAR_NAME in os.environ:
            self.debug_dir_root = os.path.join(os.environ[DEBUG_DIR_VAR_NAME], "torch_compile_debug")
        elif self.is_fbcode():
            self.debug_dir_root = os.path.join(tempfile.gettempdir(), "torch_compile_debug")
        else:
